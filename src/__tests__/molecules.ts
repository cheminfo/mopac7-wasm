 ...
 *
 * {@link EXTRA_MOLECULES} is not from that set: those four are small idealised
 * geometries written here, so that the option-coverage tests have an open shell,
 * an anion and a degenerate spectrum to run on. Each carries the bond length it
 * was built at.
 */
...
export type ExtraMoleculeName = 'methyl' | 'methane' | 'hydroxide' | 'dioxygen';
